...
/** @var array<string,string> $linkStatusMessages */
  ...
  $this->render('layouts/base', compact('pageTitle', 'nav', 'currentUser', 'flash'), function () use (
    ...
    $hasMenu = $menu !== null;
    $currentMenuId = $hasMenu ? (int)($menu['id'] ?? 0) : null;
  ...
  <div class="row g-3" data-navigation-manager>
    ...
    <div class="col-lg-4">
      <div class="card mb-3">
        ...
        <div class="list-group list-group-flush" data-navigation-menu-list>
          <?php foreach ($menus as $m): ?>
            ...
            <a class="list-group-item list-group-item-action <?= $active ? 'active' : '' ?>"
               ...
               data-navigation-menu-id="<?= $h((string)$m['id']) ?>">
          ...
          <?php endforeach; ?>
          <?php if (!$menus): ?>
            <div class="list-group-item text-secondary" data-navigation-menu-empty>Žádná menu zatím nejsou vytvořena.</div>
          <?php endif; ?>
        </div>
      </div>

      <div class="card mb-3<?= $hasMenu ? '' : ' d-none' ?>" data-navigation-edit-card>
        <div class="card-header">Upravit menu</div>
        <div class="card-body">
          <form method="post" action="admin.php?r=navigation&a=update-menu" class="mb-3" data-ajax data-navigation-edit-menu-form data-navigation-action="update-menu">
            <input type="hidden" name="csrf" value="<?= $h($csrf) ?>" data-navigation-csrf>
            <input type="hidden" name="id" value="<?= $currentMenuId ? $h((string)$currentMenuId) : '' ?>" data-navigation-menu-id-input>
            <div class="mb-3">
              <label class="form-label">Název</label>
              <input type="text" name="name" class="form-control" value="<?= $hasMenu ? $h((string)$menu['name']) : '' ?>" required data-navigation-edit-name>
            </div>
            <div class="mb-3">
              <label class="form-label">Slug</label>
              <input type="text" name="slug" class="form-control" value="<?= $hasMenu ? $h((string)$menu['slug']) : '' ?>" placeholder="např. hlavni-menu" data-navigation-edit-slug>
            </div>
            <div class="mb-3">
              <label class="form-label">Lokace</label>
              <?php if ($menuLocations): ?>
                <select name="location" class="form-select" data-navigation-edit-location>
                  <?php foreach ($menuLocations as $loc => $info): ?>
                    <?php
                      ...
                      $isDisabled = $assignedId !== null && $assignedId !== $currentMenuId;
                    ...
                    <option value="<?= $h($value) ?>"<?= $isSelected ? ' selected' : '' ?><?= $isDisabled ? ' disabled' : '' ?><?= $title !== '' ? ' title="' . $h($title) . '"' : '' ?>>
                      <?= $h((string)$info['label']) ?><?php if ($assignedId !== null && $assignedId !== $currentMenuId && $assignedName !== ''): ?> — obsazeno (<?= $h($assignedName) ?>)<?php endif; ?>
                    </option>
                  <?php endforeach; ?>
                </select>
                <div class="form-text">Umístění jsou načtena ze šablony. Každé lze přiřadit pouze jednomu menu.</div>
              <?php else: ?>
                <input type="text" name="location" class="form-control" value="<?= $hasMenu ? $h((string)$menu['location']) : 'primary' ?>" placeholder="např. primary" data-navigation-edit-location>
              <?php endif; ?>
            </div>
            <div class="mb-3">
              <label class="form-label">Popis</label>
              <textarea name="description" class="form-control" rows="3" placeholder="Volitelný popis menu" data-navigation-edit-description><?= $hasMenu ? $h((string)($menu['description'] ?? '')) : '' ?></textarea>
            </div>
            <div class="d-grid gap-2">
              <button class="btn btn-primary" type="submit">Uložit změny</button>
            </div>
          </form>
          <form method="post" action="admin.php?r=navigation&a=delete-menu" data-ajax data-confirm-modal="Opravdu smazat toto menu včetně všech položek?" data-confirm-modal-title="Smazat menu" data-confirm-modal-confirm-label="Ano, smazat" data-confirm-modal-cancel-label="Ne" data-navigation-action="delete-menu" data-navigation-delete-form>
            <input type="hidden" name="csrf" value="<?= $h($csrf) ?>" data-navigation-csrf>
            <input type="hidden" name="id" value="<?= $currentMenuId ? $h((string)$currentMenuId) : '' ?>" data-navigation-menu-id-input>
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <?php else: ?>
            <form method="post" action="admin.php?r=navigation&a=create-menu" data-ajax data-navigation-action="create-menu" data-navigation-create-menu-form>
              <input type="hidden" name="csrf" value="<?= $h($csrf) ?>" data-navigation-csrf>
              <div class="mb-3">
                <label class="form-label">Název</label>
                <input type="text" name="name" class="form-control" required data-navigation-create-name>
              </div>
              <div class="mb-3">
                <label class="form-label">Slug</label>
                <input type="text" name="slug" class="form-control" placeholder="automaticky dle názvu" data-navigation-create-slug>
              </div>
              <div class="mb-3">
                <label class="form-label">Lokace</label>
                <?php if ($menuLocations): ?>
                  ...
                  ?>
                  <select name="location" class="form-select"<?= !$hasAvailableLocation ? ' disabled' : '' ?> data-navigation-create-location>
                  ...
                  <div class="form-text" data-navigation-create-location-help>
    ...
    <div class="col-lg-8">
      <div class="card<?= $hasMenu ? ' d-none' : '' ?>" data-navigation-empty-state>
        <div class="card-body">
          <p class="mb-0 text-secondary">Vyberte existující menu nebo vytvořte nové, abyste mohli spravovat jeho položky.</p>
        </div>
      </div>

      <div class="d-flex justify-content-between align-items-center mb-3<?= $hasMenu ? '' : ' d-none' ?>" data-navigation-selected-menu-header>
        <h2 class="h5 m-0" data-navigation-selected-menu-title>
          <?= $hasMenu ? 'Položky menu „' . $h((string)$menu['name']) . '“' : 'Položky menu' ?>
        </h2>
        <a class="btn btn-outline-secondary btn-sm" href="<?= $hasMenu ? 'admin.php?r=navigation&menu_id=' . $h((string)$menu['id']) : '#' ?>" data-navigation-cancel-link>Zrušit úpravy</a>
      </div>

      <div class="card mb-3<?= $hasMenu ? '' : ' d-none' ?>" id="item-form" data-navigation-item-card>
        <div class="card-header" data-navigation-item-card-title><?= $editingItem ? 'Upravit položku' : 'Přidat položku' ?></div>
        <div class="card-body">
          <form method="post" id="navigation-item-form" action="admin.php?r=navigation&a=<?= $editingItem ? 'update-item' : 'create-item' ?>" data-ajax data-navigation-item-form data-navigation-action="<?= $editingItem ? 'update-item' : 'create-item' ?>">
            <input type="hidden" name="csrf" value="<?= $h($csrf) ?>" data-navigation-csrf>
            <input type="hidden" name="menu_id" value="<?= $hasMenu ? $h((string)$menu['id']) : '' ?>" data-navigation-field="menu_id">
            <input type="hidden" name="id" value="<?= $editingItem ? $h((string)$editingItem['id']) : '' ?>" data-navigation-field="id">
            ...
            <input type="hidden" name="link_type" id="navigation-item-link-type" value="<?= $h($currentLinkType) ?>" data-navigation-field="link_type">
            <input type="hidden" name="link_reference" id="navigation-item-link-reference" value="<?= $h($currentLinkReference) ?>" data-navigation-field="link_reference">
            <div class="row g-3">
              <div class="col-md-6">
                <label for="navigation-item-title" class="form-label">Název</label>
                <input type="text" id="navigation-item-title" name="title" class="form-control" value="<?= $editingItem ? $h((string)$editingItem['title']) : '' ?>" placeholder="např. O nás" required data-navigation-field="title">
              </div>
              <div class="col-md-6">
                <label class="form-label">URL</label>
                <div class="input-group">
                  <input type="text" id="navigation-item-url" name="url" class="form-control" value="<?= $editingItem ? $h((string)$editingItem['url']) : '' ?>" placeholder="/cesta nebo https://..." data-navigation-field="url">
                  <?php if ($hasQuickAdd): ?>
              ...
              <div class="col-md-4">
                ...
                <select name="target" class="form-select" data-navigation-field="target">
              ...
              <div class="col-md-4">
                ...
                <select name="parent_id" class="form-select" data-navigation-parent-select data-navigation-field="parent">
              ...
              <div class="col-md-4">
                ...
                <input type="number" name="sort_order" class="form-control" value="<?= $h((string)$defaultOrder) ?>" data-navigation-field="sort">
                <div class="form-text">Nižší číslo zobrazí položku výše.</div>
              </div>
              <div class="col-md-6">
                <label class="form-label">CSS třída</label>
                <input type="text" name="css_class" class="form-control" value="<?= $editingItem ? $h((string)($editingItem['css_class'] ?? '')) : '' ?>" placeholder="např. btn-primary" data-navigation-field="css_class">
                <div class="form-text">Volitelný styl odkazu (pokročilé použití).</div>
              </div>
              <div class="col-12">
                <div class="d-flex gap-2">
                  <button class="btn btn-<?= $editingItem ? 'primary' : 'success' ?>" type="submit" data-navigation-submit><?= $editingItem ? 'Uložit položku' : 'Přidat položku' ?></button>
                  <a class="btn btn-outline-secondary<?= $editingItem ? '' : ' d-none' ?>" href="<?= $hasMenu ? 'admin.php?r=navigation&menu_id=' . $h((string)$menu['id']) : '#' ?>" data-navigation-reset-link>Zrušit</a>
                </div>
              </div>
            </div>
          </form>
      ...
      <div class="card<?= $hasMenu ? '' : ' d-none' ?>" data-navigation-items-card>
        <div class="table-responsive">
          <table class="table table-sm table-hover align-middle mb-0">
            ...
            <tbody data-navigation-items-body>
              <?php foreach ($items as $it): ?>
                <?php $isEditing = $editingItem && (int)$editingItem['id'] === (int)$it['id']; ?>
                <?php $indent = max(0, (int)$it['depth']) * 16; ?>
                <tr class="<?= $isEditing ? 'table-active' : '' ?>" data-navigation-item-row data-navigation-item-id="<?= $h((string)$it['id']) ?>">
                  <td>
                    <div style="padding-left: <?= $indent ?>px">
                      <div class="fw-semibold" data-navigation-item-title><?= $h((string)$it['title']) ?></div>
                      ...
                      <?php if (!empty($it['css_class'])): ?>
                        <span class="badge text-bg-secondary mt-1" data-navigation-item-class><?= $h((string)$it['css_class']) ?></span>
                      <?php endif; ?>
                  ...
                  <td>
                    <div class="text-truncate" style="max-width:240px;" data-navigation-item-url>
                      <?= $h((string)$it['url']) ?>
                    </div>
                  </td>
                  ...
                  <td data-navigation-item-sort><?= $h((string)$it['sort_order']) ?></td>
                  <td class="text-end">
                    <div class="d-inline-flex gap-2 align-items-center">
                      <a class="admin-icon-btn" href="<?= $hasMenu ? 'admin.php?r=navigation&menu_id=' . $h((string)$menu['id']) . '&item_id=' . $h((string)$it['id']) : '#' ?>#item-form" aria-label="Upravit položku" data-bs-toggle="tooltip" data-bs-title="Upravit položku" data-navigation-edit-link>
                        <i class="bi bi-pencil" aria-hidden="true"></i>
                      </a>
                      <form method="post" action="admin.php?r=navigation&a=delete-item" class="d-inline" data-ajax data-confirm-modal="Opravdu odstranit tuto položku?" data-confirm-modal-title="Smazat položku" data-confirm-modal-confirm-label="Ano, smazat" data-confirm-modal-cancel-label="Ne" data-navigation-action="delete-item">
                        <input type="hidden" name="csrf" value="<?= $h($csrf) ?>" data-navigation-csrf>
                        <input type="hidden" name="menu_id" value="<?= $hasMenu ? $h((string)$menu['id']) : '' ?>">
              ...
              <?php endforeach; ?>
              <?php if (!$items): ?>
                <tr data-navigation-items-empty>
              ...
              <div class="modal-body">
                ...
                <ul class="nav nav-tabs" id="navigationContentTabs" role="tablist">
                  <?php foreach ($modalTabs as $key => $tab): ?>
                    ...
                    <?php $isActiveTab = !$disabled && $key === $activeTab; ?>
                    <li class="nav-item" role="presentation">
                      <button class="nav-link<?= $isActiveTab ? ' active' : '' ?><?= $disabled ? ' disabled' : '' ?>" id="navigation-content-tab-<?= $h($key) ?>" data-bs-toggle="tab" data-bs-target="#navigation-content-pane-<?= $h($key) ?>" type="button" role="tab" aria-controls="navigation-content-pane-<?= $h($key) ?>" aria-selected="<?= $isActiveTab ? 'true' : 'false' ?>" <?= $disabled ? ' tabindex="-1" aria-disabled="true"' : '' ?>>
                ...
                <div class="tab-content pt-3">
                  <?php foreach ($modalTabs as $key => $tab): ?>
                    ...
                    <?php $isActiveTab = $key === $activeTab && !empty($itemsList); ?>
                    <div class="tab-pane fade<?= $isActiveTab ? ' show active' : '' ?>" id="navigation-content-pane-<?= $h($key) ?>" role="tabpanel" aria-labelledby="navigation-content-tab-<?= $h($key) ?>">
                      <?php if ($itemsList): ?>
