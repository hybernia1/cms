...
function initDeleteAction(form) {
  const root = form.closest('[data-post-editor-root]') || document;
  const deleteButton = root.querySelector('[data-post-delete-trigger]');
  if (!deleteButton || deleteButton.dataset.postDeleteBound === '1') {
    return;
  }

  const targetFormId = deleteButton.getAttribute('data-post-delete-form');
  let deleteForm = null;
  if (targetFormId && targetFormId.trim() !== '') {
    deleteForm = document.getElementById(targetFormId);
  }
  if (!deleteForm) {
    deleteForm = root.querySelector('[data-post-delete-form]');
  }
  if (!deleteForm) {
    return;
  }

  deleteButton.dataset.postDeleteBound = '1';
